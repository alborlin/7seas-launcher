...
def client(mock_sdk):
    with patch("sevenseas.core.torbox.TorboxApi", return_value=mock_sdk), \
         patch("sevenseas.core.torbox.httpx.Client") as mock_http_cls:
        c = TorboxClient(api_key="test-key")
        c._mock_http = mock_http_cls.return_value
        return c


def test_create_torrent(client, mock_sdk):
    mock_sdk.torrents.create_torrent.return_value = MagicMock(
        data=SimpleNamespace(torrent_id=42)
    )
...
def test_check_status_returns_none_for_missing(client):
    mock_resp = MagicMock()
